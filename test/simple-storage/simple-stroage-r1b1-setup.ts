...
describe("SimpleStorageR1B1Setup", function () {
  ...
  describe("prepareInstallation", async () => {
    ...
    before(async () => {
      initData = abiCoder.encode(buildMetadata1.pluginSetupABI.prepareInstallation, [defaultInputR1B1.number]);
    });

    it("returns the plugin, helpers, and permissions", async () => {
      const nonce = await ethers.provider.getTransactionCount(simpleStorageR1B1Setup.address);
      ...
      } = await simpleStorageR1B1Setup.callStatic.prepareInstallation(dao.address, initData);
      ...
      const simpleStorageR1B1 = new SimpleStorageR1B1__factory(signers[0]).attach(plugin);

      // initialization is correct
      expect(await simpleStorageR1B1.dao()).to.eq(dao.address);
      expect(await simpleStorageR1B1.number()).to.be.eq(defaultInputR1B1.number);
    });
  });
  ...
  describe("prepareUninstallation", async () => {
    it("returns the permissions", async () => {
      ...
      const permissions = await simpleStorageR1B1Setup.callStatic.prepareUninstallation(dao.address, {
        plugin: dummyAddr,
        currentHelpers: [],
        data: EMPTY_DATA,
      });
      ...
      expect(permissions).to.deep.equal([
        [Operation.Revoke, dummyAddr, dao.address, NO_CONDITION, STORE_PERMISSION_ID],
      ]);
    });
  });
});
