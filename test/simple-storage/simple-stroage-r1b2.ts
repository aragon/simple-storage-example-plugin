...
import { ADDRESS_ONE, ADDRESS_TWO, STORE_PERMISSION_ID } from "./simple-storage-common";
...
describe("SimpleStorageR1B2", async function () {
  ...
  beforeEach(async () => {
    simpleStorageR1B2 = await deployWithProxy<SimpleStorageR1B2>(SimpleStorageR1B2);
  });

  describe("initialize", async () => {
    it("reverts if trying to re-initialize", async () => {
      await simpleStorageR1B2.initializeBuild2(dao.address, defaultInput.number, defaultInput.account);

      await expect(
        simpleStorageR1B2.initializeBuild2(dao.address, defaultInput.number, defaultInput.account),
  ...
  describe("storing", async () => {
    ...
    describe("storeAccount", async () => {
      it("reverts if sender lacks permission", async () => {
        await expect(simpleStorageR1B2.storeNumber(newNumber))
          ...
          .withArgs(dao.address, simpleStorageR1B2.address, signers[0].address, STORE_PERMISSION_ID);
      });

      it("stores the number", async () => {
        await dao.grant(simpleStorageR1B2.address, signers[0].address, STORE_PERMISSION_ID);

        await expect(simpleStorageR1B2.storeNumber(newNumber)).to.not.be.reverted;
    ...
    describe("storeAccount", async () => {
      ...
      it("stores the account", async () => {
        ...
        await expect(simpleStorageR1B2.storeAccount(newAccount)).to.not.be.reverted;
