...
describe("SimpleStorageR1B3", async function () {
  ...
  beforeEach(async () => {
    simpleStorageR1B3 = await deployWithProxy<SimpleStorageR1B3>(SimpleStorageR1B3);
  });

  describe("initialize", async () => {
    it("reverts if trying to re-initialize", async () => {
      await simpleStorageR1B3.initializeBuild3(dao.address, defaultInput.number, defaultInput.account);

      await expect(
        simpleStorageR1B3.initializeBuild3(dao.address, defaultInput.number, defaultInput.account),
      ).to.be.revertedWith("Initializable: contract is already initialized");
    });

    it("stores the number and account and emits the NumberStored and AccountStored events", async () => {
      const tx = await simpleStorageR1B3.initializeBuild3(dao.address, defaultInput.number, defaultInput.account);

      // Check events
      const numberStoredEvent = await findEvent<NumberStoredEvent>(tx, "NumberStored");
      expect(numberStoredEvent?.args.number).to.be.equal(defaultInput.number);

      const accountStoredEvent = await findEvent<AccountStoredEvent>(tx, "AccountStored");
      expect(accountStoredEvent?.args.account).to.be.equal(defaultInput.account);
    ...
    describe("storeNumber", async () => {
      it("reverts if sender lacks permission", async () => {
        await expect(simpleStorageR1B3.storeNumber(newNumber))
          ...
          .withArgs(dao.address, simpleStorageR1B3.address, signers[0].address, STORE_NUMBER_PERMISSION_ID);
      });

      it("stores the number and emits the NumberStored event", async () => {
        await dao.grant(simpleStorageR1B3.address, signers[0].address, STORE_NUMBER_PERMISSION_ID);
    ...
    describe("storeAccount", async () => {
      it("reverts if sender lacks permission", async () => {
        await expect(simpleStorageR1B3.storeAccount(newAccount))
          .to.be.revertedWithCustomError(SimpleStorageR1B3, "DaoUnauthorized")
          .withArgs(dao.address, simpleStorageR1B3.address, signers[0].address, STORE_ACCOUNT_PERMISSION_ID);
      });

      it("stores the account and emits the AccountStored event", async () => {
        await dao.grant(simpleStorageR1B3.address, signers[0].address, STORE_ACCOUNT_PERMISSION_ID);
