...
describe("SimpleStorageR1B2Setup", function () {
  ...
  describe("prepareInstallation", async () => {
    ...
    before(async () => {
      initData = abiCoder.encode(buildMetadata2.pluginSetupABI.prepareInstallation, [
        defaultInputR1B2.number,
        defaultInputR1B2.account,
      ]);
    });

    it("returns the plugin, helpers, and permissions", async () => {
      const nonce = await ethers.provider.getTransactionCount(simpleStorageR1B2Setup.address);
      ...
      } = await simpleStorageR1B2Setup.callStatic.prepareInstallation(dao.address, initData);

      expect(plugin).to.be.equal(anticipatedPluginAddress);
      expect(helpers.length).to.be.equal(0);
      expect(permissions.length).to.be.equal(1);
      expect(permissions).to.deep.equal([[Operation.Grant, plugin, dao.address, NO_CONDITION, STORE_PERMISSION_ID]]);

      await simpleStorageR1B2Setup.prepareInstallation(dao.address, initData);
      const simpleStorageR1B2 = new SimpleStorageR1B2__factory(signers[0]).attach(plugin);

      // initialization is correct
      expect(await simpleStorageR1B2.dao()).to.eq(dao.address);
      expect(await simpleStorageR1B2.number()).to.be.eq(defaultInputR1B2.number);
      expect(await simpleStorageR1B2.number()).to.be.eq(defaultInputR1B2.number);
    });
  });
  ...
  describe("prepareUninstallation", async () => {
    it("returns the permissions", async () => {
      ...
      const permissions = await simpleStorageR1B2Setup.callStatic.prepareUninstallation(dao.address, {
        plugin,
        currentHelpers: [],
        data: EMPTY_DATA,
      });
      ...
      expect(permissions.length).to.be.equal(1);
      expect(permissions).to.deep.equal([[Operation.Revoke, plugin, dao.address, NO_CONDITION, STORE_PERMISSION_ID]]);
    });
  });
  ...
  describe("prepareUpdate", async () => {
    ...
    context("from build 1", async () => {
      it("returns the permissions", async () => {
        ...
        const { initData, preparedSetupData } = await simpleStorageR1B2Setup.callStatic.prepareUpdate(
          dao.address,
          currentBuildNumber,
          {
            plugin: pluginBuild1.address,
            currentHelpers: [],
            data: ethers.utils.defaultAbiCoder.encode(buildMetadata2.pluginSetupABI.prepareUpdate.fromBuild1, [
              ADDRESS_ONE,
            ]),
          },
        );

        const expectedInitData = SimpleStorageR1B2__factory.createInterface().encodeFunctionData(
