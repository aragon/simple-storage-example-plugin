...
describe("SimpleStorageR1B1", function () {
  ...
  beforeEach(async () => {
    simpleStorageR1B1 = await deployWithProxy<SimpleStorageR1B1>(SimpleStorageR1B1);
  });

  describe("initialize", async () => {
    it("reverts if trying to re-initialize", async () => {
      await simpleStorageR1B1.initializeBuild1(dao.address, defaultInput.number);

      await expect(simpleStorageR1B1.initializeBuild1(dao.address, defaultInput.number)).to.be.revertedWith(
    ...
    describe("storeNumber", async () => {
      ...
      it("reverts if sender lacks permission", async () => {
        await expect(simpleStorageR1B1.storeNumber(newNumber))
          ...
          .withArgs(dao.address, simpleStorageR1B1.address, signers[0].address, STORE_PERMISSION_ID);
      });

      it("stores the number", async () => {
        await dao.grant(simpleStorageR1B1.address, signers[0].address, STORE_PERMISSION_ID);

        await expect(simpleStorageR1B1.storeNumber(newNumber)).to.not.be.reverted;
