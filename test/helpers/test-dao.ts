...
export async function deployDao(signer: SignerWithAddress): Promise<[DAO, Admin]> {
  ...
  await dao.grant(dao.address, admin.address, ethers.utils.id("EXECUTE_PERMISSION"));
  await dao.grant(dao.address, admin.address, ethers.utils.id("UPGRADE_PERMISSION"));
