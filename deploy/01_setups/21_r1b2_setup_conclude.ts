...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  console.log(`Concluding SimpleStorageR1B2Setup setup deployment.\n`);
  const [deployer] = await hre.ethers.getSigners();

  const {deployments, network} = hre;

  const setupDeployment = await deployments.get('SimpleStorageR1B2Setup');
  const setup = SimpleStorageR1B2Setup__factory.connect(
    setupDeployment.address,
    deployer
  );
  const implementation = SimpleStorageR1B2__factory.connect(
...
export default func;
func.tags = ['SimpleStorageR1B2Setup', 'Verify'];
